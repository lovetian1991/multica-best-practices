...
产品设计小队维护一份稳定命名的 `prd-<功能名称>.md`。首次发布上传文件，后续同名发布自动更新同一文件；OpenContent 自己维护版本，业务流程不读取历史版本。
...
发布命令返回 `internal_link`、`file_id`、`file_guid`、`folder_id`、`metadata_patch` 和 `comment`。编排器必须把 metadata patch 和 comment 写入当前 Multica Issue；任一写入失败都返回 `BLOCKED`，不能宣称 PRD 阶段完成。
